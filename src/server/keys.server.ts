...
export type KeySlot = (typeof KEY_SLOTS)[number]["slot"];
...
export async function saveApiKey(data: { slot?: string; label?: string; secret: string }) {
    await ensureTable();
...
export async function generateApiKey(data: { slot: string }) {
    await ensureTable();
...
export async function verifyApiKey(data: { slot: string }) {
    await ensureTable();
    const sql = await getSql();
...
export async function clearApiKey(data: { slot: string }) {
    await ensureTable();
    const sql = await getSql();
