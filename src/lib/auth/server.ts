...
const SHOP_HOSTS: string[] = [
  "sadhgurugemsandjewellers.com",
  "www.sadhgurugemsandjewellers.com",
];
const SHOP_ORIGINS: string[] = SHOP_HOSTS.map((h) => `https://${h}`);
const baseURL = {
  // Include loopback hosts so dynamic baseURL resolves for local email/password
  // (not only the preview wildcard). Live shop hosts so the custom domain
  // can sign in (email/password Origin check + OAuth redirect_uri).
  allowedHosts: [...previewAllowedHosts, ...SHOP_HOSTS, "localhost", "127.0.0.1", "[::1]"],
  // `auto` → trust both http:// and https:// expansions of allowedHosts
  // (preview is https; local dev is http).
  protocol: "auto" as const,
  fallback: explicitBaseURL ?? "https://www.sadhgurugemsandjewellers.com",
};
